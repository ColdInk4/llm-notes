...
AGENTS.md 是**流程规范**：回答前复核、修改前必读、审计循环、联网求证门槛、commit 约定。STYLE.md 是**写作规范**：Markdown 排版、提示块用法、术语口径、来源记录格式。两份文档在以下五处必须保持一致：(1) 禁用句式清单；(2) 提示块 5 种 alert 语义；(3) 章节末来源记录的 2 段结构（官方 / 事实声明指向；不设「课程来源」子段）；(4) GitHub 渲染安全规则（9 种公式渲染失败模式与自检 rg）；(5) 第一性原理三条硬约束与逻辑审计 logic_finding 字段口径。修改 `AGENTS.md` 后须 `cp AGENTS.md CLAUDE.md` 一并提交。
...
修改公式或含 `$` 的正文后，跑以下检测（9 种失败模式），命中即按 STYLE.md 规则修复：

```bash
# 模式1：行内开 $ 紧贴非空白字符（汉字 / 全角标点 / 字母 / 数字 / dollar-backtick 反引号），GitHub 整个公式不渲染
#         （after-类须含 0-9 与 `——实证 ch3「，$1/b$」、ch8「；$10^{21}$」开 $ 后随数字，旧 pattern 漏抓）
rg -n '[^\s*$\\`]\$[A-Za-z0-9\\(`]' <modified-markdown-files>
...
# 模式9：display 块内行首 "- "（减号+空格）被 CommonMark 解析为列表项、打断 $$ 块（+ / * 与数字. 行首同理）
python3 - <modified-markdown-files> <<'PY'
import re, pathlib, sys
for p in sys.argv[1:]:
    ...
    for i, ln in enumerate(lines, 1):
        ...
        if s.startswith('$$') and s.endswith('$$') and len(s) > 2:
            continue
        if in_dd and re.match(r'^\s*([-+*]\s|\d+\.\s)', ln):
            print(f"{p}:{i}")
PY
```

模式 2（斜体 `*…*` 图注内的裸 `$expr$`）rg 扫不到，目检图注；dollar-backtick 外层双反引号包裹（`` `` $`expr`$ `` `` 变 code span、`$` 恒字面）用 `rg -n '``\s*\$`' <modified-markdown-files>` 检测——斜体图注与普通正文都只写裸 `$`expr`$` / `$expr$`，不加外层。用 curl 抓 GitHub 页面数裸 `$` 时，排除 `aria-label="Permalink:` 属性与 `<span>$</span>` 金额 span——permalink 的字面 `$` 属页面模板自动生成的不可见属性、金额 span 是模式 4 的合法修复，均不算失败；最终判定以剥掉 `<math>` / `<math-renderer>` 标签后文本节点中的字面 `$` 为准。

命中处理：模式 1 在开 `$` 前补半角空格；模式 2 斜体图注内改裸 dollar-backtick（`$`expr`$`，不加外层双反引号），普通正文直接 `$expr$`、已有的外层包裹删掉外层；模式 3 改写为单参数命令省花括号形式（`$\hat{R}_t$` → `$\hat R_t$`）；模式 4 包 `<span>$</span>`；模式 5 星号改 `\ast`（`$h^*$` → `$h^\ast$`、 `$\|W_l\|_*$` → `$\|W_l\|_\ast$`）；模式 6 单反斜杠 `\_` 改双反斜杠 `\\_`，或改用 dollar-backtick 形式——双反斜杠命中后按段落做 CommonMark 强调配对检查（标点前缀 `_` 的两种开者形态 `\\_` 反斜杠、`'_{` 撇号都可开强调，同段 `_{` 型下标可闭强调，配对则 `<em>` 切碎两条公式），存在配对时 `\\_` 优先改写为连字符形式（`fan-in`、`fan-out`），`'_{` prime 下标把 prime 后移为 `$g_{i,t}'$`（`_` 变 close-only，LaTeX 渲染不变）；模式 7 裸 `<` 改 `\lt`（`x_{<t}` → `x_{\lt t}`）；模式 8 改 `\mathrm{...}`；模式 9 删行首减号后的空格（`- \tau` → `-\tau`）或把减号移到上一行行尾。修改图注时注意：斜体内公式必失败，粗体内公式可渲染。
...
- **6. 数字层**：同一硬件 / 模型 / benchmark 跨章一致（H100 BF16 dense 989.5 TFLOP/s、MMLU 12,032 题、Pile 334B tokens）；跨章引用的数字与那一章原文一致（Llama 3 70B tokens/param 215 vs ch10 15.6T → 223）；章节末「来源与更新记录」2 段结构（官方 / 事实声明指向；课程映射只在 `sources/cs336-2026.md`，不设「课程来源」子段）。
...
- **Sub-agent 不应自己 commit；改动留在 working tree 由主 agent commit + 复核**（Phase 14.1v2 教训，2026-09-22）。sub-agent 在 audit-fix 一体化方案下会做大量 Edit 并触发 commit（Phase 14.1v2 中 A1 / A2 / A3 / A4 / A5 / A6 / A8 / A9 / A10 / A11 / A12 / A13 / A14 / A15 共 14 个 sub-agent 中，**A8 ch8 自己 commit `a52b573`**，A1 ch1 self-commit `ddffb82`，A2 ch2 self-commit（含 2 个 image 文件 rename 落在 `f812969` commit）。**问题**：self-commit 跳过主 agent 在 commit 前必做的全量 WebFetch 复核——主 agent 必须主动 `git log` + `git show <sha>` 找回这些 commit 复核，否则 sub-agent 的「修复引入新错」会溜进 main。本轮 Phase 14.1v2 主 agent 在 self-commit 后才复核，发现：(1) **A8 ch8 `a52b573` 把 TP V §8.6.6 Adam hidden LR 写成 `1/fan_in`，主 agent 当时凭印象判「Table 3 原文是 Adam hidden LR = 1 (Θ(1))」并当作新错**——2026-09-23 curl `arxiv.org/pdf/2203.03466` PDF §4 Table 3 定案：hidden Adam LR = `1/fan_in`（灰 `(1)` 为 SP 对照）、hidden SGD LR = `1`，A8 该处实际正确；v1 被 revert 的是 SGD 行（`1/n_{l-1}` 应为 `1`），两处不同源。此例与 Phase 14.5 LPPO 同款：**主 agent 复核数字分歧时也必须 curl PDF + pdftotext 定案表格原文，禁止凭印象判 sub-agent 错**（html WebFetch 对该两栏表有列错位，曾给出与 PDF 相反的 hidden SGD 值）；(2) **A1 ch1 `ddffb82` 图 regen 内容**(实际 tiktoken 输出与 image 内容一致，✓ 通过)；(3) **A8 ch8 `a52b573` Mistral 7B 8T+ tokens 声称三源核证**——主 agent 当时 WebFetch arxiv abstract + HF model card 查不到 8T 数字，疑为 hallucinated；2026-09-23 核证闭环：**8T 归因真实**（ucstrategies.com Mistral 7B guide 原文 "8 trillion tokens (80% multilingual web text, 20% code)"），当时查不到是搜索路径问题（源在第三方行业媒体，不在 arxiv/HF）；**反而是正文「公告 blog 提及超过 1T」的归因错了**——mistral.ai 公告发布当日（2023-09-27）archive 快照起即无 trillion 句，在线版 / HF README / mistral-src README / arXiv 全文亦无，ch8 已撤该归因。教训：主 agent 判「查不到 ≠ 幻觉」，必须换源类型（行业媒体 / archive 快照）再搜一轮；被质疑的归因句与被辩护的归因句都要各自核。**正确做法**：(a) sub-agent prompt 必须显式说明「**不要自己 commit / git commit / git push**；所有改动留在 working tree，由主 agent 统一复核 + commit」；(b) 主 agent 必须在每轮 audit-fix 完成后跑 `git log` + `git status --short` 列出**所有**新 commit（包括 sub-agent self-commit 与主 agent commit）+ `git show <sha>` 复核每个 diff；(c) 发现 sub-agent 引入新错则立即 revert + 打 patch（与「全量 WebFetch 复核」节同流程）。**禁止妥协**：sub-agent 不应跳到 commit 阶段；主 agent 不应跳过对 self-commit diff 的复核直接进入下一阶段 dispatch。
- **Read PNG 错位：并行必错位、串行也可能错位（呈递层）——阻塞串行 Read 主通道 + OCR 交叉验证**（Phase 14.1 ch13 教训 2026-09-22；呈递层教训与用户拍板 2026-09-23）。Read 工具在同一消息中并行调用多张 PNG 时，返回的图像内容可能与所请求的文件名不对应——本轮 Phase 14.1 中，主 agent 在单条消息里并行 Read `13-4-6 / 13-4-7 / 13-4-10 / 13-4-12` 四张 PNG，返回的第一张实际是 s1 Table 1（应该属于 13-4-12）而非 R1 训练流水线（应该是 13-4-6）。A13 也基于类似的并行 Read 错位报告「图 13.4-6 = 简化 4 步 + Key differences」并据此改了 4 张图 caption——全部反向错位（A13 fix 后 4 张图 caption 与文件实拍图意全错）。**正确做法**——**严禁任何 PNG 并行 Read**：(a) 同一张图只 Read 一次，不重复；(b) 一条消息里**只能 Read 1 张 PNG**，等 transcript 拿到结果并做文件名 vs 图意二次核对后，再起下一条消息 Read 下一张；(c) 每张图 Read 后立即在 transcript 上做**文件名 vs 实际图意**二次核对，不靠 memory；(d) 对图意与 caption 不一致的图片，必须**再单独 Read 一次**才能下结论，避免并行错位引发的 caption 错位 fix。**反例**：本轮 1 条消息里并行 Read 4 张 PNG → 主 agent 与 A13 都错位 → A13 据此改了 4 张图 caption → 4 处全部反向（最终 revert 到 HEAD 原版）。**禁止妥协**：哪怕「2-3 张也不并行」，因为无法保证 2 张以上不异步错位。**2026-09-23 升级（ch3 12 张 pending 图审计实证）**：(a) **阻塞式串行（一条消息 1 张、等返回、核完再读下一张）也曾错位**——session jsonl 中 Read 返回的 base64 与磁盘文件 SHA 13/14 SAME（字节层正确），但模型对话中「看到」的图像与存档不符，损坏发生在 harness 呈递层，并发控制堵不住；(b) 可靠通道 = 本地 OCR（`rapidocr-onnxruntime` 直接跑磁盘文件）或 jsonl base64 解码 SHA 比对；(c) **用户拍板（2026-09-23）**：图审计**主通道用阻塞式串行 Read**（图像可观测性优于 OCR 文本，原话「就要求阻塞read吧，我感觉read读出来可观一些」），每张 Read 后必须与 OCR 结果交叉验证，**双通道一致才下 verdict**——单 Read 不单独定案，单 OCR 判不了图意 / caption 语义 / 删图判定；任一通道冲突即回退 OCR-only，按「文件名 vs OCR 文本」定案并复盘呈递层故障。
- **笔记任何位置不留「待核验 / 不确定 / 需复核 / tentative」类元叙述**（ch3 fresh audit 教训，2026-09-15）。核验工作外移到 chat / sub-agent transcript；sandbox 拦截一手 URL 时由主 agent 交用户在 IDE 浏览器或 curl 取回内容再核验；未核到的字段不写入笔记，已写入的撤回。章节末「来源与更新记录」只有 2 段结构（官方 / 事实声明指向），无「课程来源」子段（2026-09-23 起废除，课程映射只写 `sources/cs336-2026.md`），也无「不确定项」子段（曾用 4 段结构 + tentative 子段，已废除）。commit 前 `rg "待.*核|待核|不确定|需核|需复核|缺口|以.*为准|本节以|未能核"` 命中清零。
- **WebFetch 摘要 ≠ 完整核证（Phase 14.5 教训，2026-09-22）**。单次 `WebFetch https://arxiv.org/html/{id}` 只返回 abstract 与论文摘要式片段，不一定包含论文 Table / § 完整数字。Phase 14.5 中主 agent 复核 ch12 §12.4.3 Singhal et al. LPPO StackExchange 数字时，**只跑了 WebFetch 摘要式返回，看到 Table 6 Sim Pref 给「Stack 43%」就误以为 LPPO 数字是 43%，把笔记原 LPPO Stack 58% + sub-agent 改的 59% 都 revert 了**——实际 Table 6 的 43% 是 R-DA intervention vs STND PPO 的数字，**不是 LPPO vs SFT baseline 的数字**；LPPO 实际是 Singhal §3.2 Table 2 vs SFT 50% baseline 的 Stack = **59%**，与 PPO Stack = 58% 接近。**正确做法**：(a) 当笔记数字与 WebFetch 摘要返回数字存在分歧时，**不要立即改笔记**——先 `curl https://arxiv.org/pdf/{id} -o /tmp/x.pdf && pdftotext -layout /tmp/x.pdf /tmp/x.txt` 拿论文 PDF 全文，确认 (i) baseline 描述（vs SFT 50% / vs STND PPO 50% / vs R-DA 等不同 intervention）、(ii) Table 完整行 / 列 / 数字、(iii) 章节归属（§3.2 Table 2 / Table 6 / 附录 C 等）；(b) 单次 WebFetch 摘要式返回标注 `web_evidence_fetched: partial` 或 `truncated` 时，必须扩展到 PDF 完整正文；(c) 即使「sub-agent fix 后 WebFetch 摘要说另一个数字」，也可能是 sub-agent 拿到 PDF 完整正文而主 agent 复核只看到摘要——必须 curl PDF / pdftotext 验证 sub-agent 的 `fetch_url` 与 `fetch_snippet` 是不是真的一手核证；(d) 主 agent 全量 WebFetch 复核遇到数字分歧时，**优先信任 sub-agent 拿到一手页面后的 fix**，并在 commit message 里写明 baseline 与表格出处；(e) 「数字分歧需确认 baseline / 表格出处再改笔记」——不能仅凭 WebFetch 摘要式返回就改笔记。**反例**：Phase 14.5 commit `3d7ab79` 把 Phase 14.5 sub-agent 改对的「Stack LPPO 59%」revert 回错的「Stack LPPO 58%」，并把 PPO Stack 58% 改成错的 57%——主 agent WebFetch 摘要式返回错把 Table 6 R-DA 数字 43% 当作 LPPO 数字。**禁止妥协**：只要笔记数字与 WebFetch 摘要存在分歧，必须先 PDF 完整正文核证 baseline + 表格出处 + 数字；不能凭「摘要里说了一个不同的数字」就直接改笔记或 revert sub-agent。
...
| 审计 7 层第 6 层（数字） | STYLE.md「来源与日期」+「章节末『来源与更新记录』约定」 | 同一硬件 / 模型 / benchmark 跨章一致；2 段结构（官方 / 事实声明指向；课程映射只在 `sources/cs336-2026.md`；不确定项不留在笔记，由 chat 流程核验） |
...
| 修改后自检 4（GitHub 渲染安全 rg） | STYLE.md「公式与排版」渲染条目 +「HTML」金额 span 例外 | 9 种渲染失败模式（开 `$` 紧贴含数字与反引号开头 / 斜体图注与 dollar-backtick 外层包裹 / `}_`·`|_` 切碎 / 字面金额 / math 内裸 `^*`·`_*` / math 内 `\_` 单反斜杠 / display 内裸 `<` / `\operatorname` 宏禁用 / display 行首 `- ` 列表项打断）的 rg 与修复规则一一对应；图注用裸 `$`expr`$`（不加外层双反引号）、金额用 `<span>$</span>`、星号用 `\ast`、下划线用 `\\_`（标点前缀 `_` 含 `\\_` 与 prime `'_{` 两种开者，同段 `_{` 配对时 `\\_` 改连字符 `fan-in`、prime 后移 `g_{i,t}'`）、小于号用 `\lt`、函数名用 `\mathrm`、display 行首减号去后随空格 |
