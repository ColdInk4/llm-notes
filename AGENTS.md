...
- **并行 Read PNG 可能异步错位返回**（Phase 14.1 ch13 教训，2026-09-22）。Read 工具在同一消息中并行调用多张 PNG 时，返回的图像内容可能与所请求的文件名不对应——本轮 Phase 14.1 中，主 agent 在单条消息里并行 Read `13-4-6 / 13-4-7 / 13-4-10 / 13-4-12` 四张 PNG，返回的第一张实际是 s1 Table 1（应该属于 13-4-12）而非 R1 训练流水线（应该是 13-4-6）。A13 也基于类似的并行 Read 错位报告「图 13.4-6 = 简化 4 步 + Key differences」并据此改了 4 张图 caption——全部反向错位（A13 fix 后 4 张图 caption 与文件实拍图意全错）。**正确做法**：(a) 同一张图只 Read 一次，不重复；(b) 同一消息里**最多并行读 2-3 张 PNG**；超过 4 张时拆成多个串行消息或一次只读 1 张；(c) 每张图 Read 完拿到结果后立即在 transcript 上做**文件名 vs 实际图意**二次核对，不靠 memory；(d) 对图意与 caption 不一致的图片，必须**先单独再 Read 一次**（顺序单 Read）才能下结论，避免并行错位引发的 caption 错位 fix。
