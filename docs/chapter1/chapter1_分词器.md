...
比较三种策略时，应固定同一批 Unicode 文本、同一 round-trip 检查和同一词表预算，再记录 token 数、bytes per token、未知符号率与编码时间。这样观察到的差异才来自切分粒度，而不是语料规模或实现速度不同。
