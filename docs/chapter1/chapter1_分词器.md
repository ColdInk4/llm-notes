...
这个 trace 展示了 BPE 的两条账本同时变化：token 序列从 18 个 byte 压到 12 个 token，词表从 256 增加到 259。第 2 轮的 `(257, 32)` 还跨过了词边界——只按频率贪心合并时，高频词会连着它后面的空格一起进词表；预分词规则的作用就是先划开 chunk 边界：词尾和后面的空白落在不同 chunk 里，这类跨边界合并就不会发生。
...
两套规则都按类别划开 chunk，BPE 合并只发生在同一 chunk 内部，词尾不会连上后随的空白或标点；
