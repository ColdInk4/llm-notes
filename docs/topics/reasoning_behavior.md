...
本专题作为第 9 章、第 12 章和第 13 章之后的横向补充，讨论 LLM 推理作为可观察行为时如何产生、被训练和被使用。

第 9 章关心 TTFT、throughput、KV cache、PagedAttention 和调度；本专题关心模型为什么会展开思考，CoT、多路径采样、prompt、后训练和工具搜索如何改变可见推理行为，以及这些方法的能力边界和风险。两者的连接点是推理预算：更复杂的推理行为通常会消耗更多 generation tokens、KV cache、scheduler 容量和 rollout 推理成本。

日常使用大语言模型（LLM）时，简单聊天、代码辅助和复杂问题求解都会触发模型生成一段看起来有条理的回答。复杂问题往往需要中间步骤；一些人类几乎能一眼判断的简单问题，也可能触发模型生成较长的推理文本。这说明推理行为首先是一种可观察的生成行为，需要从能力、解码、训练目标和系统成本几个角度一起分析。

近期值得关注的研究案例包括：

- Anthropic 的 Claude 相关案例记录了模型参与 Donald Knuth 研究过的图论猜想推导过程，见 [Knuth 的记录](https://www-cs-faculty.stanford.edu/~knuth/papers/claude-cycles.pdf)。
- Brenner（Harvard SEAS 应用数学与应用物理）、Cohen-Addad（Google Research）和 Woodruff（CMU 与 Google Research 联合）在 2026 年 3 月 5 日发布的 [论文](https://arxiv.org/pdf/2603.04735)中，结合 Gemini Deep Think 大语言模型与 Tree Search 框架及自动化数值反馈，识别出 6 种不同的解析方法（最优雅的一种以 Gegenbauer 多项式展开核函数），求解宇宙弦引力辐射功率谱问题的精确解析解。
- Tony Feng 的 [arXiv:2601.23245](https://arxiv.org/abs/2601.23245) *Eigenweights for arithmetic Hirzebruch Proportionality* 是数学论文，作者在 *Declaration of AI Usage* 一节中写明"核心数学内容（Type A / Type C / Type D 等经典群的 eigenweight 公式与证明）由内部推理代理（a (custom) AI Agent built upon Gemini Deep Think）完整生成"；Type B 的 eigenweight 沿用 prior work [FYZ25a] 计算，AI 代理仅生成 A/C/D 等新结果。人类作者 Tony Feng 的工作是搭建该推理代理、把代理输出重写成论文形式并撰写引言。这条记录说明 LLM 已以"作者代理"形式进入专业数学研究流程；代理承担数学构造，作者承担代理搭建与论文改写。

![专题图 1 简单数值比较中的过度推理示例](images/reasoning-01-api-number-comparison.png)

*专题图 1 简单数值比较中的过度推理示例*

专题图 1 的问题只是比较 $9.169$ 和 $9.6$ 的大小，但模型仍然可能生成一段分步解释。这类例子把能力问题和成本问题放在同一张账本里：推理文本可以提高可读性和可验证性，也会占用更多输出 token、更长上下文和更多 serving 资源。分析 LLM 推理能力时，需要同时追问五个问题：

- 预训练阶段的模型分布中是否已经包含正确推理轨迹？
- CoT 和多路径采样怎样改变这些轨迹被取出的概率？
- prompt 模板能否稳定地诱导模型采用更合适的问题分解方式？
- SFT、RLHF、RLVR 等后训练如何改变搜索偏好、输出规范和奖励风险？
- 外部工具和搜索系统怎样扩展模型的信息来源、动作空间和系统成本？
...
它的优点是简单、低延迟、易于批处理；限制是每一步都只看当前局部最大概率，无法保证整条序列的联合概率最优。一个两步例子可以说明这个问题。词表中有 A、B、C、D 四个候选 token，目标是生成两步路径。
...
一些研究表明，更强的基座模型通常更容易从 RL 或偏好优化中获益。一个直观解释是：后训练依赖基座模型先提供足够好的初始策略。如果底座能力太弱，奖励信号也很难把采样轨迹推到高质量解题区域。
...
偏好优化的关键风险是实验条件依赖性：偏好评估容易受长度、风格和标注者分布影响。RLVR 的系统代价来自 on-policy 训练需要大量 rollouts。每条 rollout 都要执行慢速 generation；训练框架与推理框架之间的切换、长 CoT 造成的 ragged batch，以及 verifier 打分都会成为系统瓶颈。

可以把后训练理解为三件事的组合：提高好轨迹的采样概率，约束输出格式和风格，降低错误轨迹的表面吸引力。它不会绕过预训练能力基础；它会改变能力被调用、搜索和呈现的方式。

## CoT：推理长度、有效深度与系统成本

随着智能体和长流程任务变多，推理成本里越来越大的一部分来自更长上下文和更长思维链。CoT 自 2022 年被系统提出后，显著提升了数学和复杂推理任务表现；同时，它也引出一个工程问题：更长的推理链在什么条件下带来有效计算，什么时候只是更贵的输出。

> [!WARNING]
> 更长的思维链不自动等于更强的推理能力。后训练可以通过奖励函数鼓励模型生成更长、更详细的 CoT；
> 如果奖励只偏好长度或形式，模型可能学会冗长表达、重复推导和奖励黑客。

更长的 CoT 会拉长 generation 阶段，并让后续每一步 attention 读取更长历史。在线服务里，这会表现为更高 latency、更大的 KV cache 占用和更难打包的 ragged batch；训练 RLVR 时，则会表现为 rollout 吞吐下降。
...
DTR 仍然是统计性指标。完整推理链需要浅层组织语言，也需要深层完成关键判断；未来如果要用 DTR 控制 CoT，还需要结合 token 不确定性、路径一致性、外部验证器和任务难度。Qwen 3 的混合思维模式（hybrid thinking modes）也沿着这条线索前进：通过 thinking / non-thinking 数据混合、特殊终止标记和 thinking budget，让模型在不同任务上调节推理长度。（Qwen 3 官方博客称为 hybrid thinking modes，并把 Thinking Mode 与 Non-Thinking Mode 作为该 hybrid 框架下的两种模式；"thinking mode fusion" 仅出现在后训练第 3 阶段名称，不是面向用户的特性名。）

## Prompt 引导：用输入结构约束模型行为

Prompt 的作用是改变模型的条件分布。它不会改写模型参数，却能通过任务描述、示例、格式约束和角色设定，让模型更容易进入合适的解题轨迹。对第一次阅读者来说，prompt 可以理解为推理阶段的轻量控制接口。
...
模型参数在训练结束后通常保持固定，因此参数化知识天然存在时间滞后和覆盖范围限制。外部工具把模型能力从“只依赖参数记忆”扩展到“可以检索、调用 API、执行代码、观察环境并整合结果”。
...
工具搜索也会增加系统成本。一次回答可能包含多段 prompt prefill、多次 generation、长上下文 KV cache、工具返回内容压缩、引用去重和 scheduler 排队。它在能力上接近“推理 + 搜索 + 验证”的组合；在系统上则回到 [第 9 章 推理系统](../chapter9/chapter9_推理系统.md) 的 serving 账本。
...
一个值得继续追问的问题是：推理能力的提升来自预训练中已经形成的潜在结构，还是来自后训练阶段对搜索、验证和表达方式的重新组织？更稳妥的判断是二者共同作用。预训练提供可迁移结构和候选轨迹；后训练、prompt、解码和工具系统决定这些轨迹以什么概率、成本和可靠性被调用。
...
- [Stanford 持续进化 AI](https://zitongyang.github.io/slides/ZitongYang_defense_slides.pdf)
