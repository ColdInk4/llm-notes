...
- 描述 PPO → GRPO → Dr. GRPO 的算法演进主线，特别是 value model 去掉后 advantage 如何估计、std / length 两类偏置如何引入与如何修正。
- 把 GRPO 训练流程分成七个步骤：rollout、reward 计算、group 内 z-score、advantage 估计、ratio clipping、KL penalty、policy 更新。
...
传统的 RLHF 依赖人类对模型输出的成对偏好判断（如 “A 比 B 好”）。RLHF 的算法细节（reward model / value model / KL 约束 / 37 implementation details）已在 [第 12 章 §12.5 偏好优化与 DPO 系列](../chapter12/chapter12_大模型基本训练流程.md) 展开；本章聚焦的是把 RLHF 范式换成 RLVR 之后的工程边界。这类数据有三类扩展问题：
...
- Increasing overall rewards：总奖励上升。`kl_sum_seq` 这个名字有点误导性，它实际上代表的是每个序列的塑形奖励（shaped rewards）的总和，这个“塑形奖励” = 任务奖励（如数学题答对得高分） + KL 惩罚项（负值），衡量模型的整体表现是否在变好。
- Incl. reward model：任务奖励上升。这个指标衡量的是模型直接从奖励模型那里获得的任务奖励，不包括 KL 惩罚。它反映了模型在核心任务上的表现，比如回答问题的准确性、遵循指令的能力等。
- Negative KL rewards：负的 KL 奖励。这个指标就是 KL 惩罚项，也就是前面提到的 $-\beta \cdot KL$ 。说明模型确实在探索和改进，但没有失控。这是 PPO 算法“近端”思想的体现——允许一定程度的偏离，但限制其幅度。

这三条曲线共同描绘了一个健康的 PPO 训练过程：模型在奖励模型的引导下，逐步学会生成更好的回复，同时通过 KL 惩罚保持一定的稳定性，避免过度偏离初始的良好行为。
...
$$
D_{\mathrm{KL}} \left( \pi_\theta \| \pi_{\text{ref}} \right) = \frac{\pi_{\text{ref}}(o_i|q)}{\pi_\theta(o_i|q)} - \log \frac{\pi_{\text{ref}}(o_i|q)}{\pi_\theta(o_i|q)} - 1
$$

这是一个采样点上的近似 KL 散度公式，用来衡量当前策略 $\pi_\theta$ 和参考策略 $\pi_{\text{ref}}$ 在生成特定输出 $o_i$ 时的概率差异。
...
- RFT (Reinforcing Fine-Tuning)：只奖励最终答案正确性，生成过程不单独打分。
- Online RFT：训练过程中动态采样和更新，减少对固定样本的依赖。
...
- GRPO 曲线高于 RFT，说明组内相对 advantage 比只看最终正确性更容易提供有效更新信号。
- 过程监督在部分区间带来额外收益，但它要求更细的标注或验证器，扩展成本也更高。
- 曲线平滑度来自组内归一化和在线采样的共同作用；换到其他任务时仍要重新检查 reward 分布、采样温度和 KL。

### 13.3.2 GRPO 的潜在缺陷：长度偏差 (Length Bias)

GRPO 的简化带来明显工程收益，也引入了需要单独处理的 bias。
...
这个问题可以放回 policy gradient 的 baseline 条件中理解：只依赖状态的 baseline 可以保持梯度无偏；组内标准差来自同组 rollouts，会改变不同问题的权重。Dr. GRPO 与 leave-one-out 风格变体的目标，是保留方差降低的收益，同时减少由标准差归一化和长度归一化引入的偏差。

![图 13.3-5 Dr. GRPO 与无偏梯度变体](images/13-3-5-dr-grpo-unbiased-variant.png)

*图 13.3-5 Dr. GRPO 与无偏梯度变体*

Dr. GRPO 的核心改动在于移除了 GRPO 原有的 $\frac{1}{|o_i|}$ （响应长度归一化）和 $\frac{1}{\text{std}(\{R(q, o_1), \dots, R(q, o_G)\})}$ （标准差归一化），从而修正了 GRPO 中存在的响应长度偏见和问题难度偏见。

图 13.3-4 的右侧比较了 GRPO 和 Dr. GRPO 训练过程中的 reward 与 output length。移除长度归一化后，错误回答不再通过变长来削弱负 advantage，token 效率也更容易稳定。


#### 2. **长度偏见**

GRPO 的长度偏差来自 token 级更新权重。目标函数中，单个响应 $o_i$ 在时间步 $t$ 的梯度更新会涉及：
...
- $\hat{A}_{i,t}$ ：是优势函数，其计算方式为 $\hat{A}_{i,t} = \frac{R(q, o_i) - \text{mean}(\{R(q, o_1), \dots, R(q, o_G)\})}{\text{std}(\{R(q, o_1), \dots, R(q, o_G)\})}$ ，其中 $R(q, o_i)$ 是响应 $o_i$ 的回报（reward）。

当 $\hat{A}_{i,t} > 0$ 时，较短的正确回答会得到更大的 token 级正更新，模型被推向更短的正确答案。当 $\hat{A}_{i,t} < 0$ 时，较长的错误回答会把负 advantage 摊薄到更多 token 上，单个 token 承受的惩罚变小。

这种机制会让错误回答通过变长来降低惩罚强度，训练曲线上就可能出现“越错越长”的现象。

GRPO 的偏差可以拆成两类：标准差项会放大太容易或太难的问题，长度归一化会改变正负 advantage 在长短回复上的 token 级更新强度。工程上看到输出长度上升时，需要同时检查 reward variance、正确/错误回复长度、KL 和 format reward。

![图 13.3-6 GRPO 的长度与难度权重偏差](images/13-3-6-grpo-length-bias.png)

*图 13.3-6 GRPO 的长度与难度权重偏差*

### 13.3.3 Group z-score 与 std-normalization 的偏差来源

GRPO 的优势估计对一个 prompt 内的 $G$ 条回复做组内归一化：

$$
A_i = \frac{r_i - \mathrm{mean}(\mathbf{r})}{\mathrm{std}(\mathbf{r})}
$$

其中 $\mathbf{r}=(r_1,\dots,r_G)$ 是同一 prompt 下多条回复的奖励序列。这条公式的关键变量是 group 内的标准差 $\mathrm{std}(\mathbf{r})$。当一个 prompt 的所有回复要么全部正确、要么全部错误时（"all-correct / all-wrong"），组内方差趋近于 0，归一化后 advantage 容易在数值上放大或退化；更细的偏差来源还有：

- **长度方差耦合**：长回复倾向于在同一 prompt 上拿到更大方差，进而通过 std 项放大 advantage 的绝对值，让长回复更可能被优化。
- **难度方差耦合**：组内同时出现全对 / 全错回复时，难度信息被错误地归到 advantage 数值上。
- **response 长度 vs std 联合偏差**：回复长度与组内方差通常正相关，这会让"长 + 方差大"的问题在归一化后被相对抬高。

[Dr. GRPO](https://arxiv.org/abs/2503.20783)（Liu et al., 2025）的修正方向是去掉长度归一化（避免 token 级 advantage 在长短回复上的强度变化）和 std 归一化（避免方差趋零带来的数值尖峰，论文原文 "simply remove the 1/|o_i| and std(...) normalization terms"）。具体选择仍是工程经验题：去掉长度归一化后 short / long 回答的优化强度会重新分配，需要按任务特性决定是否补偿。

> 工程经验：当输出长度持续上涨时，先检查 prompt 内正确率与长度方差的关系；长度偏差和 z-score 偏差常常同时出现，需要在 reward shape、normalization 和 verifier 三个层面分别评估。
...
R1 系列沿用了 DeepSeekMath 论文中的 GRPO 思路。

![图 13.4-2 GRPO 与其他算法的对比](images/13-4-2-deepseek-grpo-comparison.png)

*图 13.4-2 GRPO 与其他算法的对比*
...
![图 13.4-3 DeepSeek-R1-Zero 与 OpenAI o1 的推理基准对比](images/13-4-3-r1-zero-o1-benchmarks.png)

*图 13.4-3 DeepSeek-R1-Zero 与 OpenAI o1 的推理基准对比*
...
*图 13.4-4 DeepSeek-R1-Zero 的 AIME 准确率与响应长度*

图 13.4-4 把 AIME 准确率和平均响应长度放在一起。训练过程中，准确率上升常伴随更长的响应；这些额外 token 中包含检查、回溯和替代路径搜索。这个现象说明可验证奖励会改变模型分配 test-time compute 的方式，但长度增长本身也需要和算法偏差、格式奖励一起分析。

![图 13.4-5 DeepSeek-R1-Zero 的 aha moment 案例](images/13-4-5-r1-zero-aha-moment.png)

*图 13.4-5 DeepSeek-R1-Zero 的 aha moment 案例*

图 13.4-5 展示的是一条生成轨迹中的可见行为变化：模型先给出一个解法，再用“等等”一类词触发自我检查并修正路线。这类文本现象可以作为训练过程中的行为信号；证据强度更高的判断要结合准确率、长度、reward 和错误类型一起看。

##### 需要谨慎理解的地方

GRPO 的长度偏差已经在图 13.3-4 和图 13.3-6 中出现过。理解 R1-Zero 的长度增长时，需要把“更多 test-time compute 带来推理收益”和“有偏目标改变长度偏好”分开检查。

基础模型早已表现出“aha moment”：

![图 13.4-6 DeepSeek-V3-Base 的 aha moment 现象](images/13-4-6-deepseek-v3-base-aha-example.png)

*图 13.4-6 DeepSeek-V3-Base 的 aha moment 现象*
...
![图 13.4-7 DeepSeek-R1 训练流程](images/13-4-7-deepseek-r1-training-pipeline.png)

*图 13.4-7 DeepSeek-R1 训练流程*
...
![图 13.4-8 DeepSeek R1 相对 R1-Zero 的训练差异](images/13-4-8-r1-zero-r1-differences.png)

*图 13.4-8 DeepSeek R1 相对 R1-Zero 的训练差异*
...
![图 13.4-9 生成可读回答的摘要提示](images/13-4-9-r1-readable-answer-prompt.png)

*图 13.4-9 生成可读回答的摘要提示*
...
![图 13.4-10 DeepSeek-R1 与其他模型的比较](images/13-4-10-deepseek-r1-benchmarks.png)

*图 13.4-10 DeepSeek-R1 与其他模型的比较*
...
![图 13.4-11 DeepSeek-R1 蒸馏模型与其他模型的比较](images/13-4-11-r1-distillation-benchmarks.png)
...
*图 13.4-13 s1 使用 1k 高质量样本提升数学推理*
...
上海交通大学的刘鹏飞团队在[LIMO: Less is More for Reasoning](https://arxiv.org/abs/2502.03387)中也得到相似结论：用不到千条高质量、带长思维链的样本在 Qwen2.5-32B-Instruct 上做 SFT，就能显著提高数学推理表现。论文首版（v1, 2025-02-05）§3.3.1 Question Selection 写 "ultimately yielded 817 carefully curated problems from an initial pool of tens of millions of candidates"，abstract 同步写 "With merely 817 curated training samples"；2025-07 修订版（v2/v3）把最终样本数调整为 800，abstract 改用 "only 1% of the training data required by prior approaches" 的相对口径。

![图 13.4-14 LIMO 使用 800 个高质量样本提升数学推理](images/13-4-14-limo-small-data-math.png)

*图 13.4-14 LIMO 使用 800 个高质量样本提升数学推理*
...
![图 13.4-15 LIMR 与其他模型的性能比较](images/13-4-15-limr-rl-benchmarks.png)

*图 13.4-15 LIMR 与其他模型的性能比较*
...
参考：Moonshot AI, *Kimi k1.5: Scaling Reinforcement Learning with LLMs*, [arXiv:2501.12599](https://arxiv.org/abs/2501.12599)。

#### 长思维链推理策略

![图 13.4-16 Kimi k1.5 的长 CoT 结果](images/13-4-16-kimi-long-cot-results.png)

*图 13.4-16 Kimi k1.5 的长 CoT 结果*
...
![图 13.4-17 Kimi k1.5 的参考策略 RL 目标](images/13-4-17-kimi-rl-objective.png)

*图 13.4-17 Kimi k1.5 的参考策略 RL 目标*
...
其中 $R_{\text{len}}(i)$ 对应代码里的 `len_reward(i)`， $\ell_i$ 表示第 $i$ 个回复的长度， $\ell_{\min}$ 和 $\ell_{\max}$ 分别对应 `min_len` 与 `max_len`。并且：
...
![图 13.4-18 Kimi k1.5 的长度控制](images/13-4-18-kimi-length-control.png)

*图 13.4-18 Kimi k1.5 的长度控制*
...
- 仅剔除模型在 **best-of-8 sampling**（同 prompt 采 8 次）下就已经能稳定答对的题目——这是 **reward-hacking filter**，去掉"无需 CoT 也能猜对"的样本，避免奖励被这些题目稀释。该过滤策略是 Kimi k1.5 论文 [arXiv:2501.12599](https://arxiv.org/abs/2501.12599) §2.1 RL Prompt Set Curation 特有方法（原文："If the model predicts the correct answer within N attempts, the prompt is considered too easy-to-hack and removed. We found that setting N=8 can remove the majority easy-to-hack prompts."）；DeepSeek-R1 报告 [arXiv:2501.12948](https://arxiv.org/abs/2501.12948) 不包含该 best-of-N filtering——其数据筛选走的是 rejection sampling 取正确答案的路径，不做 N-attempt hack detection。难度过滤是另一条独立的 pass-rate-based curriculum（按 $(1-\text{success\_rate})$ 分配采样比例）

奖励：

- 对于代码——采用具有 ground truth 解的问题，生成新的测试用例
- 对于数学——使用 **约 800k 条样本**训练一个 CoT 奖励模型（[arXiv:2501.12599](https://arxiv.org/abs/2501.12599) §2.3.5 "Reward Modeling for Math" 给出 "an equally large dataset of about 800k CoT-labeled examples"；Classic RM 同样约 800k 数据点），专门做答案等价性检查（同一最终答案在数学上有多种表达形式，需要专用 reward model 而非纯规则 reward）

#### Scaling 结果

Kimi-k1.5 的公开结果展示了长思维链 SFT 与后续 RL 组合在若干推理任务上的收益。这里更值得长期记住的是训练流程与长度控制思路：先用高质量长 CoT 数据塑造可读推理轨迹，再用 RL 和奖励设计把这种轨迹转化为更稳定的任务表现。

![图 13.4-19 Kimi k1.5 与其他方法的性能对比](images/13-4-19-kimi-benchmark-comparison.png)

*图 13.4-19 Kimi k1.5 与其他方法的性能对比*

图 13.4-20 训练准确率和响应长度随阶段变化。

![图 13.4-20 Kimi k1.5 训练准确率与长度变化](images/13-4-20-kimi-training-accuracy-length.png)

*图 13.4-20 Kimi k1.5 训练准确率与长度变化*

#### 消融实验

![图 13.4-21 Kimi k1.5 与 ReST 的策略梯度消融](images/13-4-21-kimi-rest-ablation.png)

*图 13.4-21 Kimi k1.5 与 ReST 的策略梯度消融*
...
Qwen3 的公开材料表明，思考模式融合与 thinking budget 控制已经从“研究技巧”走向了更系统的后训练设计。图 13.4-22 的跨模型对比由各家技术报告的评测设置、提示模板与采样参数共同限定。

![图 13.4-22 Qwen3 与其他模型的性能比较](images/13-4-22-qwen3-benchmark-comparison.png)

*图 13.4-22 Qwen3 与其他模型的性能比较*

Qwen3 的后训练流程围绕两类控制展开：

![图 13.4-23 Qwen3 系列模型的后训练管线](images/13-4-23-qwen3-posttraining-pipeline.png)

*图 13.4-23 Qwen3 系列模型的后训练管线*
...
数据进入 RL 前先做难度过滤：移除模型在没有 CoT 时就能答对的问题，移除与验证数据过于相似的内容，并人工检查 CoT 是有效推理还是猜测。过滤后的强化学习样本很少。Qwen 3 技术报告 §4.2 ([arXiv:2505.09388](https://arxiv.org/abs/2505.09388)) 给出 Reasoning RL 阶段共 **3,995 query-verifier pairs**，以 GRPO 训练 170 RL steps；这一数字仅覆盖 Reasoning RL 阶段，不含其它 RL 阶段样本。样本品质、验证可靠性和采样策略比样本条数本身更关键。

#### Thinking Mode Fusion

思考模式融合的训练入口，是混合带标签的非思考和思考数据。

![图 13.4-24 思考模式融合阶段的 SFT 数据示例](images/13-4-24-qwen3-thinking-mode-sft.png)

*图 13.4-24 思考模式融合阶段的 SFT 数据示例*

思考模式融合让模型同时见到带 `<think>` 的长推理回答和直接回答。模型学会两种模式后，也能处理中间状态：当 thinking budget 用尽时，系统截断 `<think>` 过程并插入停止思考指令，模型再基于已经生成的推理给出最终答案。训练分布中的模式融合已经能承担主要控制作用，预算控制模块只是可选实现。

> [!NOTE]
> **Thinking 与 non-thinking 拆分的现实边界**：Qwen 3（[arXiv:2505.09388](https://arxiv.org/abs/2505.09388)）阶段用 single model + thinking_budget API 实现，把 `/think` 与 `/no_think` 模式放进同一个 checkpoint；这一设计在 Qwen 3 报告里被列为 Thinking Mode Fusion 的成功范式。后续更大规模模型沿用同样思路还是改成多 checkpoint 部署，取决于具体后训练工程取舍（多模式融合的训练分布设计、长 CoT 标注规模、推理 serving 的工作负载隔离），并不存在统一的必然拆分路线。该取舍说明 hybrid thinking 路线在 prompt 层是稳定的，但能否在更大规模上继续保持单 checkpoint 融合，要看每家团队的具体训练配方。

#### 测试时间扩展（Test time scaling，TTS）

图 13.4-25 比较 AIME'24、AIME'25、LiveCodeBench v5 和 GPQA Diamond 上的 Pass@1。横轴是 thinking budget，曲线区分 thinking 与 non-thinking 模式，用来观察测试时计算量如何换取任务表现。

![图 13.4-25 Qwen3-235B-A22B 随 thinking budget 的性能](images/13-4-25-qwen3-thinking-budget.png)

*图 13.4-25 Qwen3-235B-A22B 随 thinking budget 的性能*

#### 不同阶段的组成

图 13.4-26 展示 Qwen3-32B 在不同后训练阶段的性能变化。

![图 13.4-26 Qwen3-32B 在不同后训练阶段的性能](images/13-4-26-qwen3-posttraining-stages.png)

*图 13.4-26 Qwen3-32B 在不同后训练阶段的性能*

需要注意的是，通用型 RLHF 可能会让数学和 STEM 能力略有下降。


#### Qwen3-Coder-Next：agentic RL

Qwen3-Coder-Next 是 agentic RL 的代表性案例（[Hugging Face 模型卡](https://huggingface.co/Qwen/Qwen3-Coder-Next)，80B 总参 / 3B 激活，原生 262,144 上下文；架构与 Qwen3-Next-80B-A3B 一致，建立在 mid-trained Qwen3-Next 上）。它通过 coding 和 agent 环境继续后训练。中期数据包括 repository-level GitHub 数据、带 RAG 上下文的 pull request、Common Crawl 中的 text+code 文档、合成 coding QA、coding agent 轨迹和 fill-in-the-middle 数据。

这条线和第 10 章的数据工程相互呼应：SWE-smith、SWE-Zero 这类数据把真实仓库、可执行环境、强模型轨迹和过滤检查组合起来，训练样本本身已经带有评估与系统工程结构。
...
> [!NOTE]
> Qwen3-Coder-Next 通过专家蒸馏把单轮 QA、web dev、UX 与代码任务混合到 RL 流程：强模型对代码语义有内部 value model，在没有显式执行反馈的情况下也能推断任务结果——这正是 SWE-Zero 这类无执行数据训练可行的根本原因；同时 Qwen3-Coder-Next 又使用执行反馈 verifier，两条路线混合以兼顾数据规模与奖励可靠性。中期训练的仓库级 GitHub 数据规模为 6,000 亿 token，SWE-bench 式自动化环境构建覆盖约 80 万个任务。
>
> **命名说明**：Qwen3.5 系列（0.8B / 2B / 4B / 9B / 27B / 35B-A3B / 122B-A10B / 397B-A17B）中并不存在 "Qwen 3.5 Next Coder" 成员；本节统一使用官方命名 Qwen3-Coder-Next。

![图 13.4-27 Qwen3-Coder-Next 的专家蒸馏路线](images/13-4-27-qwen3-coder-distillation.png)

*图 13.4-27 Qwen3-Coder-Next 的专家蒸馏路线*

## 本章总结与下章衔接

RLVR 把后训练主线从"人类偏好 → 偏好模型"换成"可验证奖励 → policy gradient"。本章按可验证奖励的脉络梳理：先讲 PPO 在语言模型后训练中的工程痛点（37 implementation details 那一类博文为代表性吐槽），再讲 GRPO（去掉 value function，advantage 用组内 reward z-score 估计），再到 Dr. GRPO 修正 z-score 引入的 std / length 两类偏置。三个案例研究展示当前 RLVR 的工程骨架：DeepSeek-R1 用规则奖励 + 冷启动长 CoT + 大规模 SFT 数据 + 蒸馏；Kimi k1.5 用 long-CoT SFT + best-of-8 难度过滤 + 长度奖励 λ ∈ [-0.5, 0.5] + 大规模 CoT reward model；Qwen 3 用 RLVR 例子 + repository-level data + 自动化 SWE-bench 任务 + Qwen3-Coder-Next 的 agentic RL 蒸馏。
...
- 论文与文档来源（查阅日期 2026-09-03）：
  - [DeepSeekMath, arXiv:2402.03300](https://arxiv.org/abs/2402.03300) §4.1.2 / §4.1.3 / §4.2（outcome vs process supervision、GRPO+OS / GRPO+PS、约 144K 条 GSM8K + MATH CoT 训练数据）
  ...
  - [Dr. GRPO, arXiv:2503.20783](https://arxiv.org/abs/2503.20783) §2.3 "Aha Moment Already Appears in Base Models Including DeepSeek-V3-Base"、§3.1 "GRPO Leads to Biased Optimization"、§3.2 Dr. GRPO
  - [Kimi k1.5, arXiv:2501.12599](https://arxiv.org/abs/2501.12599) §2.1 RL Prompt Set Curation（N=8 easy-to-hack 过滤）、§2.3.5 Reward Modeling for Math（约 800k CoT 标注样本）
  - [Qwen3, arXiv:2505.09388](https://arxiv.org/abs/2505.09388) §4.2 Reasoning RL（3,995 query-verifier pairs、170 RL steps、AIME'24 70.1 → 85.1）
  - [LIMO, arXiv:2502.03387](https://arxiv.org/abs/2502.03387) v1 §3.3.1（817）与 v2/v3（800）
