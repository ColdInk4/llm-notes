...
传统的 RLHF 依赖人类对模型输出的成对偏好判断（如 “A 比 B 好”）。reward model、value model 和 KL 约束这套算法组件已在 [第 12 章 §12.4 大模型训练的第三个阶段：对齐人类偏好（RLHF）](../chapter12/chapter12_大模型基本训练流程.md) 展开；本章聚焦的是把 RLHF 范式换成 RLVR 之后的工程边界。这类偏好数据有三类扩展问题：
