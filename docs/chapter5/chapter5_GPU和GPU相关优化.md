...
本节回答三个前置问题：GPU 为什么会从图形处理器演化成 AI 训练与推理的核心加速器，CPU 和 GPU 在设计目标上各偏向哪一端，以及 A100/H100/H200/B200 四代硬件在 SM 数、HBM 容量与带宽、L2 上有什么量级差异。读完后读者应能把一张规格表转译成「这一代解决了上一代什么瓶颈」的判断，建立数量级感。

在深度学习概念没有火起来之前，GPU 更常被理解为图形处理器。图形渲染天然包含大量相似的小计算：同一套几何和像素规则要重复应用到许多顶点、三角形和像素上。这种 workload 正好适合用大量简单计算单元并行处理。

以 3D 模型为例，模型表面通常由许多三角形组成。系统保存顶点、边和材质等紧凑信息，渲染时并行计算中间像素和光照结果。
...
| --- | --- | --- | --- | --- |
| SM 数 | 108 | 132 | 132（[NVIDIA CUDA Programming and Optimization](https://developer-blogs.nvidia.com/wp-content/uploads/2024/08/CUDA-Programming-and-Optimization.pdf) GPU Overview 页 "NVIDIA H200 SXM" 规格区 "132 SMs"） | B200 整卡 148 个 SM（[NVIDIA 技术博客](https://developer.nvidia.com/blog/?p=114188) 规格对照表 "NVIDIA Blackwell B200 (Cloud)" 列 SM count 148；全封装 2 个 GB100 die）；每 SM 128 个 FP32 core（[Cornell Virtual Workshop](https://cvw.cac.cornell.edu/gpu-architecture/horizon-gpus-blackwell-b200/b200_sm) B200 SM 页 "128 FP32 CUDA cores"），合计 18,944 个 FP32 CUDA core（[TechPowerUp](https://www.techpowerup.com/gpu-specs/b200-sxm6.c4210) B200 条目 18,944 shading units，148 × 128 = 18,944） |
...
A100、H100、B200 在后文共同出现，分别承担基线剖面、Hopper 特性和 Blackwell 新增能力的参照。学习这些规格时要建立数量级和瓶颈判断，无须背某个 SKU 的完整参数表。
...
图 5.1-3 给出板卡层面的组件（显存、供电、显示接口、金手指与 PCB）。顺着板上的显卡核心往里看，支撑 SM、warp 和 shared memory 分析的关键规格分三层：
...
下面把 GA100 拆成三个粒度，作为 SM、warp 与 shared memory 抽象的物理参照。§5.2 SM 执行模型沿用这三个抽象。
...
| **带宽** | **128 bytes/周期**（32 bank × 4 bytes，无 bank conflict） |
| **延迟** | **20-40 周期** |
...
| 特性 | 参数  说明 |
|------|------------|
| **物理位置** | **SM 内统一寄存器文件**（每个线程按 thread ID 分配私有寄存器条目） |
| **容量** | **256 KB/SM** (A100) |
| **带宽** | **单周期供给**（warp 操作数直读 register file，无 cache 层级） |
| **延迟** | **1 周期** (零开销) |
| **编程控制** | **完全自动** (编译器分配) |
| **可见性** | **线程私有** |
...
| 内存类型 | 速度 | 容量 | 成本(<span>$</span>/GB)（估计） |
|----------|------|------|------------|
| 寄存器 | 1 周期 | 256 KB/SM | <span>$</span>1,000,000 |
| 共享内存 | 20 周期 | 192 KB/SM | <span>$</span>100,000 |
| L2 缓存 | 200 周期 | 40 MB | <span>$</span>10,000 |
| HBM2e 显存 | ~290 周期 | 80 GB | <span>$</span>100 |
...
图 5.5-3 是 roofline 模型，横轴是**算术强度**（图中标作 Operational Intensity），表示 FLOPs 与数据搬运字节数的比率。算术强度高时，计算设备在进行大量计算而数据移动相对较少；算术强度低时，数据移动占主导。纵轴是**吞吐量（Throughput）**，表示计算设备每秒可以完成的浮点运算次数。
...
其中算术强度（arithmetic intensity）是 FLOPs / bytes。逐元素 ReLU、GeLU 这类算子通常搬运一次数据只做少量计算，容易落在 memory-bound 区域；大矩阵乘法通过数据复用把每个 byte 对应的 FLOPs 拉高，更容易靠近 compute-bound 屋顶。
...
GPU 采用 SIMT（单指令多线程）执行架构，**同一线程束（Warp）内的所有线程同步执行相同指令**（仅操作数据不同）。NVIDIA GPU 的 warp 固定包含 32 个线程；AMD GPU 中与 warp 对应的执行小组称为 wavefront，包含 64 个线程。
...
> - **重计算**：当 activation 本身不大或 kernel 已受算力限制、带宽尚未吃满时，recomputation 只会增加 FLOPs 而不减少 HBM 往返，反而变慢。
...
>
> 每条技巧都先要回答「当前 kernel 受 memory-bound 还是 compute-bound 限制」，再决定要不要上——roofline 与算术强度这条主线以可达 FLOPs/s 为判断依据，kernel 的实际 runtime 由瓶颈一侧的资源决定。
...
总的来说，重计算丢弃 $S_1$ 与 $S_2$ 的保存，在反向传播中重新算一遍；它权衡的是计算时间与内存读写成本，计算成本小于读写成本时更划算。
...
FlashAttention 在数学上与标准 attention 完全等价（仅差浮点舍入），输出与标准 attention 逐元素一致。可运行 [flash_attention.ipynb](examples/flash_attention.ipynb) 对比两者的计算结果。


![图 5.7-2 FlashAttention 的内存层级与分块数据通路](images/5-7-2-flashattention-runtime-memory.png)
...
- [NVIDIA CUDA Programming and Optimization（developer-blogs PDF）](https://developer-blogs.nvidia.com/wp-content/uploads/2024/08/CUDA-Programming-and-Optimization.pdf) — GPU Overview 页 "NVIDIA H200 SXM" 规格区 "132 SMs"（同页 4th Gen Tensor Cores、NVLink 900 GB/s）；PDF 创建日期 2024-04-08；2026-09-23 查阅。
- [NVIDIA 技术博客：Scaling Autonomous AI Agents and Workloads with NVIDIA DGX Spark](https://developer.nvidia.com/blog/?p=114188) — 规格对照表列 "NVIDIA Blackwell B200 (Cloud)"：compute capability SM 10.0、SM count 148、HBM3e、带宽 ~8 TB/s；2026-09-23 查阅。
- [Cornell Virtual Workshop: Inside a Blackwell B200 SM](https://cvw.cac.cornell.edu/gpu-architecture/horizon-gpus-blackwell-b200/b200_sm) — 每 SM "128 FP32 CUDA cores"（另 128 INT32、64 FP64）；2026-09-23 查阅。
- [TechPowerUp: NVIDIA B200 SXM6](https://www.techpowerup.com/gpu-specs/b200-sxm6.c4210) — shading units 18,944、SM count 148（聚合数据库口径，与 NVIDIA blog 的 148 及 148 × 128 算术一致）；2026-09-23 查阅。
...
- §5.1.3 硬件表（A100/H100/H200/B200 在 SM 数、HBM 容量与带宽、L2 cache 的量级差异）：NVIDIA Blackwell tuning guide §1.4.2.2 + NVIDIA H100 / A100 / H200 datasheets；A100 108 SM 见 [NVIDIA Ampere architecture in-depth blog](https://developer.nvidia.com/blog/nvidia-ampere-architecture-in-depth/)；B200 整卡 148 SM 见 [NVIDIA 技术博客 DGX Spark 规格对照表](https://developer.nvidia.com/blog/?p=114188)、每 SM 128 FP32 core 见 [Cornell Virtual Workshop B200 SM 页](https://cvw.cac.cornell.edu/gpu-architecture/horizon-gpus-blackwell-b200/b200_sm)、合计 18,944 FP32 core 见 [TechPowerUp B200 条目](https://www.techpowerup.com/gpu-specs/b200-sxm6.c4210)（148 × 128 = 18,944）；H200 132 SM 见 [NVIDIA CUDA Programming and Optimization](https://developer-blogs.nvidia.com/wp-content/uploads/2024/08/CUDA-Programming-and-Optimization.pdf)；H100 132 SM per GPU 与每 SM 128 FP32 core 见 [NVIDIA H100 Tensor Core GPU Architecture 白皮书](https://dam-cdn.nvd.orangelogic.com/AssetLink/705n6ur546g0uk43w0117r17n8042d73.pdf)；GB200 NVL72 反推 186 GB/GPU 的总 HBM3e 13.4 TB / 72 GPU 见 [第 2 章 §2.4 计算效率](../chapter2/chapter2_pytorch与资源核算.md) 与 [第 7 章 §7.1.4 GPU、TPU 和数据中心拓扑](../chapter7/chapter7_分布式训练.md)。
