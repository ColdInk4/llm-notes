...
现在观察设备上通信与计算的时序当计算第1层时，通信单元已**预取**第2层参数；计算第2层时，通信单元又**预取**第3层参数，形成**流水线执行**。这种设计使得通信几乎被完全隐藏，虽然**理论上需要三倍通信量**，但实际效率损失可能仅为**10%-20%**。
